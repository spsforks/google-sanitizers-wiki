...
Instead of generating independent tags for `x` and `y` with two `IRG` instructions, we generate a single **base tagged pointer** for the function, and then use `ADDG` to obtain a tagged address of the second local variable. This trick helps reduce register pressure in function with multiple local variables: in the worst case, a function with N independently tagged variables would require N live registers just to store their addresses. The use of the base tagged pointer allows materializing tagged address of any local variable with a single arithmetic instruction.

### Tagging and initializing at the same time
...
- `STGP Xt, Xt2, [Xn]`, similar to `STP`, writes 16 bytes of memory at `[Xn, Xn + 16]` and updates allocation tag to match.
- `STZG` and `STZ2G` zero and tag 16 and 32 bytes of memory, correspondingly.
