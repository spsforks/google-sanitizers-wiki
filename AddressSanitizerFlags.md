...
| fast\_unwind\_on\_fatal | 0                 | Use fast unwinder when found a bug. The fast one relies on frame pointers, while slow one does not. See also [AddressSanitizerCallStack](AddressSanitizerCallStack) |
| fast\_unwind\_on\_malloc | 1                 | Same as `fast_unwind_on_fatal`, but control unwinding inside `malloc`/`free`, i.e. performance critical. See also [AddressSanitizerCallStack](AddressSanitizerCallStack) |
| exitcode | 1                 | Call `_exit(exitcode)` on error.  |
| abort\_on\_error | 0                 | If `1`, call `abort()` instead of `_exit(exitcode)` on error.|
| allow\_user\_poisoning  | 1                 | Allow/disallow [AddressSanitizerManualPoisoning](AddressSanitizerManualPoisoning) |
...
| alloc\_dealloc\_mismatch | 1                 | When set to `1`, checks for malloc()/new/new[]() vs. free()/delete/delete[]() mismatches. Default: 0 on Mac and Windows, 1 otherwise |
| detect\_stack\_use\_after\_return | 0                 | If 1, will try to detect [AddressSanitizerUseAfterReturn](AddressSanitizerUseAfterReturn) errors |
| min\_uar\_stack\_size\_log | 16                | Controls the size of Fake Stack for [AddressSanitizerUseAfterReturn](AddressSanitizerUseAfterReturn) detector |
| max\_uar\_stack\_size\_log | 20                | Controls the size of Fake Stack for [AddressSanitizerUseAfterReturn](AddressSanitizerUseAfterReturn) detector |
...
| handle\_segv | 1                 | When set to 1, [AddressSanitizer](AddressSanitizer) installs its own handler for SIGSEGV |
...
| check\_initialization\_order | 0                 | When set to 1, detect existing [InitOrderFiasco](AddressSanitizerExampleInitOrderFiasco) problems. |
...
| strict\_init\_order | 0                 | When set to 1, [InitOrderFiasco](AddressSanitizerExampleInitOrderFiasco) also finds potential init order problems. |
