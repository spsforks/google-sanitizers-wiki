...
|:----|:--------|:------------|
| -fsanitize=address |         | Enable [AddressSanitizer](AddressSanitizer.md) |
...
Most run-time flags are passed to [AddressSanitizer](AddressSanitizer.md) via `ASAN_OPTIONS` environment variable like this:
```
...
| fast\_unwind\_on\_fatal | 0                 | Use fast unwinder when found a bug. The fast one relies on frame pointers, while slow one does not. See also [AddressSanitizerCallStack](AddressSanitizerCallStack.md) |
| fast\_unwind\_on\_malloc | 1                 | Same as `fast_unwind_on_fatal`, but control unwinding inside `malloc`/`free`, i.e. performance critical. See also [AddressSanitizerCallStack](AddressSanitizerCallStack.md) |
| exitcode | 1                 | Call `_exit(exitcode)` on error.  |
| abort\_on\_error | 0                 | If `1`, call `abort()` instead of `_exit(exitcode)` on error.|
| allow\_user\_poisoning  | 1                 | Allow/disallow [AddressSanitizerManualPoisoning](AddressSanitizerManualPoisoning.md) |
...
| detect\_stack\_use\_after\_return | 0                 | If 1, will try to detect [AddressSanitizerUseAfterReturn](AddressSanitizerUseAfterReturn.md) errors |
| min\_uar\_stack\_size\_log | 16                | Controls the size of Fake Stack for [AddressSanitizerUseAfterReturn](AddressSanitizerUseAfterReturn.md) detector |
| max\_uar\_stack\_size\_log | 20                | Controls the size of Fake Stack for [AddressSanitizerUseAfterReturn](AddressSanitizerUseAfterReturn.md) detector |
...
| handle\_segv | 1                 | When set to 1, [AddressSanitizer](AddressSanitizer.md) installs its own handler for SIGSEGV |
...
| check\_initialization\_order | 0                 | When set to 1, detect existing [InitOrderFiasco](AddressSanitizerExampleInitOrderFiasco.md) problems. |
...
| strict\_init\_order | 0                 | When set to 1, [InitOrderFiasco](AddressSanitizerExampleInitOrderFiasco.md) also finds potential init order problems. |
...
| coverage | 0                 | Enables coverage collection. See [AddressSanitizerCoverage](AddressSanitizerCoverage.md). |
| coverage\_dir | ./                | Controls the path at which coverage is saved. |
| coverage\_direct | 0                 | Enables writing coverage directly to a memory-mapped file (compared to once at the end of the program). See [AddressSanitizerCoverage](AddressSanitizerCoverage.md). |
| coverage\_pcs | 1                 | If 1, will dump the coverage as PCs. See [AddressSanitizerCoverage](AddressSanitizerCoverage.md). |
| coverage\_bitset | 0                 | If 1, will dump the coverage as bitset. See [AddressSanitizerCoverage](AddressSanitizerCoverage.md). |
| detect\_odr\_violation | 2                 | If > 0, detect [ODR Violations](AddressSanitizerOneDefinitionRuleViolation.md) |
